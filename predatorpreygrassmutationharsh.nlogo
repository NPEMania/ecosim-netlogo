globals [
  ...
  max_lifespan         ;; maximum age upto which an agent can live
...
to setup
  ...
  set grass-regrowth-time 150
  set max_lifespan 3000
  ...
  [
    ...
    if random 2 = 0  ;;half the sheep are male
      [ set gender 0
  ...
  [
    ...
    let target one-of sheep with [ distance self < 5 ]
    if target != nobody [
...
to move-wolf
  ...
  [
    rt random-float 50
    lt random-float 50
    create-link-with one-of sheep with [ distance self < 5 ] [
      hide-link
    ]
  ]
  fd stride-length
...
to reproduce-sheep  ;; sheep procedure
  reproduce sheep-reproduce sheep-stride-length-drift gender
end

to reproduce-wolves  ;; wolf procedure
  reproduce wolf-reproduce wolf-stride-length-drift gender
end

to reproduce [reproduction-chance drift turtle-gender] ;; turtle procedure
  ;; throw "dice" to see if you will reproduce

  if turtle-gender = 1 and random-float 100 < reproduction-chance and energy > min-energy [
...
to maybe-die  ;; turtle procedure
  ...
  if energy < 0 or lifespan >= max_lifespan[ die ]
end
...
to grow-sheep-and-wolf  ;; turtle procedure for growth of turtle in size.
  if size = 1 [
    ifelse lifespan >= 1000
      [ set size 2 ]
      [ set lifespan lifespan + 1 ]
  ]
 if size = 2 [
    ifelse lifespan >= 2000
      [ set size 3 ]
      [ set lifespan lifespan + 1 ]
    ]
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "if any? sheep\n[plot mean[gender] of sheep]"
